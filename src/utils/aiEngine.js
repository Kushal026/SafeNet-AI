...
// ============================================================
export function analyzeAppPermissions(appName, permissions) {
    const perms = permissions.map(p => p.toLowerCase().trim());

    const riskMatrix = {
        'camera': { risk: 'medium', reason: 'Can capture photos and video' },
        'microphone': { risk: 'medium', reason: 'Can record audio at any time' },
        'location': { risk: 'high', reason: 'Tracks physical location' },
        'contacts': { risk: 'high', reason: 'Access to all contact information' },
        'sms': { risk: 'critical', reason: 'Can read/send messages, used for 2FA bypass' },
        'call logs': { risk: 'high', reason: 'Access to communication patterns' },
        'storage': { risk: 'medium', reason: 'Can read/write files on device' },
        'overlay': { risk: 'critical', reason: 'Can display content over other apps (clickjacking)' },
        'device admin': { risk: 'critical', reason: 'Full device control capabilities' },
        'accessibility': { risk: 'critical', reason: 'Can read screen content and simulate taps' },
        'background': { risk: 'medium', reason: 'Runs even when app is closed' },
        'bluetooth': { risk: 'low', reason: 'Access to nearby Bluetooth devices' },
        'wifi': { risk: 'low', reason: 'View WiFi network information' },
        'biometric': { risk: 'high', reason: 'Access to fingerprint/face authentication' },
    };

    // Dangerous combinations
    const dangerousCombos = [
        { perms: ['camera', 'microphone'], label: 'Surveillance Risk', detail: 'Camera + Microphone enables covert recording', severity: 'critical' },
        { perms: ['contacts', 'sms'], label: 'Data Exfiltration', detail: 'Can steal contacts and intercept SMS (2FA)', severity: 'critical' },
        { perms: ['location', 'contacts'], label: 'Stalkerware Pattern', detail: 'Can track you and know your contacts', severity: 'high' },
        { perms: ['overlay', 'accessibility'], label: 'Keylogger Pattern', detail: 'Can capture passwords and overlay fake login screens', severity: 'critical' },
        { perms: ['device admin', 'storage'], label: 'Full Device Control', detail: 'Can lock, wipe and read all your data', severity: 'critical' },
        { perms: ['background', 'location'], label: 'Constant Tracking', detail: 'Can track location 24/7 without interaction', severity: 'high' },
        { perms: ['camera', 'microphone', 'location'], label: 'Total Surveillance', detail: 'Can see, hear, and track you at all times', severity: 'critical' },
    ];

    const foundCombos = dangerousCombos.filter(combo =>
        combo.perms.every(p => perms.some(up => up.includes(p) || p.includes(up)))
    );

    const permDetails = perms.map(p => {
        const key = Object.keys(riskMatrix).find(k => p.includes(k) || k.includes(p));
        return key ? { name: p, ...riskMatrix[key] } : { name: p, risk: 'unknown', reason: 'Unknown permission' };
    });

    const riskCounts = { critical: 0, high: 0, medium: 0, low: 0, unknown: 0 };
    permDetails.forEach(pd => riskCounts[pd.risk] = (riskCounts[pd.risk] || 0) + 1);

    let privacyScore = 100;
    privacyScore -= riskCounts.critical * 25;
    privacyScore -= riskCounts.high * 15;
    privacyScore -= riskCounts.medium * 8;
    privacyScore -= riskCounts.low * 2;
    privacyScore -= foundCombos.filter(c => c.severity === 'critical').length * 20;
    privacyScore -= foundCombos.filter(c => c.severity === 'high').length * 10;
    privacyScore = Math.max(0, privacyScore);

    let riskLabel, recommendation;
    if (privacyScore <= 25) {
        riskLabel = 'CRITICAL RISK';
        recommendation = `This app "${appName}" requests extremely invasive permissions. Unless this is a highly trusted security/system app, UNINSTALL IT immediately. Consider reporting to your app store.`;
    } else if (privacyScore <= 50) {
        riskLabel = 'HIGH RISK';
        recommendation = `"${appName}" requires unusual permissions. Only install if from a verified developer. Consider revoking non-essential permissions in Settings > Apps.`;
    } else if (privacyScore <= 70) {
        riskLabel = 'MODERATE RISK';
        recommendation = `"${appName}" has some elevated permissions. Review each permission and deny those not needed for core functionality.`;
    } else {
        riskLabel = 'LOW RISK';
        recommendation = `"${appName}" has a reasonable permission set. Proceed with installation but review periodically.`;
    }

    return { privacyScore, riskLabel, permDetails, foundCombos, recommendation, riskCounts };
}
