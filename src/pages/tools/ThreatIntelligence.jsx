...
const SEVERITY_COLORS = { CRITICAL: '#64b5f6', HIGH: '#64b5f6', MEDIUM: '#64b5f6', LOW: '#64b5f6' };
        ...
        <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '2rem 1.5rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '2rem', flexWrap: 'wrap', gap: '1rem' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                    <div style={{ padding: '0.625rem', borderRadius: '10px', background: 'rgba(100,181,246,0.15)', border: '1px solid rgba(100,181,246,0.3)' }}>
                        <BarChart3 size={22} style={{ color: '#64b5f6' }} />
                    </div>
                    <div>
                ...
                <button onClick={fetchData} disabled={loading} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.5rem 1rem', borderRadius: '8px', background: 'rgba(100,181,246,0.1)', border: '1px solid rgba(100,181,246,0.25)', color: '#64b5f6', fontSize: '0.78rem', cursor: 'pointer', fontWeight: 600 }}>
            ...
            {loading && !data && (
                <div style={{ textAlign: 'center', padding: '4rem' }}>
                    <Loader size={40} style={{ color: '#64b5f6', animation: 'spin 1s linear infinite', margin: '0 auto', display: 'block' }} />
                </div>
            )}
            ...
            {data && (
                <>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '0.875rem', marginBottom: '1.5rem' }}>
                        {[
                            { label: 'Threats Today', value: data.global_stats.threats_today?.toLocaleString(), color: '#64b5f6' },
                            { label: 'Phishing URLs (24h)', value: data.global_stats.phishing_urls_24h?.toLocaleString(), color: '#64b5f6' },
                            { label: 'Malware Samples (24h)', value: data.global_stats.malware_samples_24h?.toLocaleString(), color: '#64b5f6' },
                            { label: 'Active Campaigns', value: data.global_stats.active_campaigns, color: '#64b5f6' },
                        ].map((stat, i) => (
                    ...
                    <div style={{ padding: '1.5rem', borderRadius: '14px', background: 'rgba(13,17,23,0.7)', border: '1px solid rgba(0,212,255,0.12)', backdropFilter: 'blur(16px)', marginBottom: '1.25rem' }}>
                        ...
                        <ResponsiveContainer width="100%" height={240}>
                            <AreaChart data={data.attack_trends} margin={{ top: 0, right: 0, left: -20, bottom: 0 }}>
                                <defs>
                                    <linearGradient id="gradPhishing" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#64b5f6" stopOpacity={0.3}/><stop offset="95%" stopColor="#64b5f6" stopOpacity={0}/></linearGradient>
                                    <linearGradient id="gradMalware" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#64b5f6" stopOpacity={0.3}/><stop offset="95%" stopColor="#64b5f6" stopOpacity={0}/></linearGradient>
                                    <linearGradient id="gradRansom" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#64b5f6" stopOpacity={0.3}/><stop offset="95%" stopColor="#64b5f6" stopOpacity={0}/></linearGradient>
                                </defs>
                                ...
                                <Area type="monotone" dataKey="phishing" stroke="#64b5f6" strokeWidth={2} fill="url(#gradPhishing)" name="Phishing" />
                                <Area type="monotone" dataKey="malware" stroke="#64b5f6" strokeWidth={2} fill="url(#gradMalware)" name="Malware" />
                                <Area type="monotone" dataKey="ransomware" stroke="#64b5f6" strokeWidth={2} fill="url(#gradRansom)" name="Ransomware" />
                            </AreaChart>
                        </ResponsiveContainer>
                    </div>
                            ...
                            <ResponsiveContainer width="100%" height={200}>
                                <BarChart data={data.industry_targets} margin={{ top: 0, right: 0, left: -25, bottom: 0 }} layout="vertical">
                                    ...
                                    <Bar dataKey="attacks" name="Attacks" radius={[0, 4, 4, 0]}>
                                        {data.industry_targets.map((_, i) => <Cell key={i} fill={'#64b5f6'} />)}
                                    </Bar>
                                </BarChart>
                            </ResponsiveContainer>
                        </div>
                    ...
                    <div style={{ padding: '1.5rem', borderRadius: '14px', background: 'rgba(13,17,23,0.7)', border: '1px solid rgba(0,212,255,0.12)', marginBottom: '1.25rem' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1.25rem' }}>
                                <AlertOctagon size={16} style={{ color: '#64b5f6' }} />
                                <span style={{ fontWeight: 700, fontSize: '0.875rem', color: '#e2e8f0' }}>Live Threat Alerts</span>
                                <div style={{ marginLeft: 'auto', fontSize: '0.65rem', padding: '0.15rem 0.5rem', borderRadius: '20px', background: 'rgba(100,181,246,0.15)', color: '#64b5f6', fontWeight: 700 }}>LIVE</div>
                            </div>
                        ...
                        return (
                            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '0.875rem', marginBottom: '0.75rem' }}>
                                ...
                                <div style={{ flex: 1, height: '8px', borderRadius: '4px', background: 'rgba(255,255,255,0.05)' }}>
                                    <div style={{ height: '100%', width: `${pct}%`, borderRadius: '4px', background: '#64b5f6', boxShadow: `0 0 8px #64b5f680`, transition: 'width 1.5s ease' }} />
                                </div>
                                <span style={{ fontSize: '0.72rem', color: '#64b5f6', fontFamily: 'JetBrains Mono, monospace', width: '60px', textAlign: 'right', flexShrink: 0 }}>{m.detections.toLocaleString()}</span>
                            </div>
                        );
                    })}
                </div>
